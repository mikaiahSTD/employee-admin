...
public class TicketPdfGenerator {
  ...
  public byte[] generate(SendEmailSubscriptionValidated event) throws IOException {
    try (PDDocument document = new PDDocument();
      ...
      try (PDPageContentStream content = new PDPageContentStream(document, page)) {
        ...
        y = writeLine(content, margin, y, "Cours", event.getCourseTitle());
        y =
            writeLine(
                content,
                margin,
        ...
        if (event.getCourseStartDate() != null) {
          y =
              writeLine(
                  content, margin, y, "Début", DATE_FORMAT.format(event.getCourseStartDate()));
        }
